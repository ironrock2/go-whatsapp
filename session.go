...
func (wac *Conn) Login(qrChan chan<- string, message string) (Session, error) {
	...
	//oldVersion=8691
	login := []interface{}{"admin", "init", []int{0, 3, 225}, []string{message, "go-whatsapp"}, session.ClientId, true}
...
func (wac *Conn) RestoreSession(session Session, message string) (Session, error) {
	...
	//admin init
	init := []interface{}{"admin", "init", []int{0, 3, 225}, []string{message, "go-whatsapp"}, session.ClientId, true}
